...
class MY_Model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->tb_blog_category     = BLOG_CATEGORIES;
        $this->tb_blog_posts_lang   = BLOG_POSTS_LANG;
        $this->tb_role_permission   = ROLE_PERMISSIONS;
        $this->tb_staffs            = STAFFS;
        $this->tb_staffs_activity   = STAFFS_LOGS;
    ...
    public function fetch_search_items($select = "*", $table = "", $where = "", $like = [], $return_array = true)
    {
        $result = null;
        $this->db->select($select);
        if ($where != "") {
        ...
        if ($like) {
            $this->db->like($like['field'], $like['query']);
        }
        $query = $this->db->get($table);
        if ($return_array) {
            $result = $query->result_array();
    ...
    public function send_mail_template($template = [], $user_id_or_email = '', $from_email_data = [])
    {
        ...
        if (is_numeric($user_id_or_email)) {
            $user_info = $this->get("email, timezone", USERS, ['id' => $user_id_or_email]);
            if (empty($user_info)) {
        ...
        try {
            ...
            if ($email_protocol_type == "smtp" && $smtp_server != "" && $smtp_port != "" && $smtp_username != "" && $smtp_password != "") {
                ...
                // ✅ limit timeout
                $mail->Timeout = 10;
                $mail->set('SMTPConnectTimeout', 10);
    ...
    {
        // if (segment(1) == base64_decode('Y3Jvbg==') || segment(2) == base64_decode('Y3Jvbg==')) {
