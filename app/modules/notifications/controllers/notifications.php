...
class notifications extends My_UserController
{
    ...
    public function view($id = "")
    {
        ...
        $items_notification_message = $this->main_model->list_items(['notification_id' => $id], ['task' => 'list-items-notification-message']);
        $data = array(
            "controller_name"       => $this->controller_name,
            "item"                  => $item,
            "items_notification_message"  => $items_notification_message,
        );
        $this->template->set_layout('user');
    ...
    public function store()
    {
        ...
        $default_pending_notification_per_user = get_option('default_pending_notification_per_user', 2);
        if ($items_pending_number >= $default_pending_notification_per_user && $default_pending_notification_per_user != 0) {
            _validation('error', 'The number of pending notifications has been limited');
        }
    ...
    public function store_message()
    {
        ...
        $task   = 'add-item-notification-massage';
