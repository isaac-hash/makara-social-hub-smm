    ...
    private function save_order($table, $data_orders, $user_balance = "", $total_charge = "", $more_params = [])
    {
        ...
        try {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                // Send order notification email (safe block)
                try {
                    ...
                    send_mail_error_log(["status" => "error", "message" => $e->getMessage()]);
                }

                // Send new manual order email (safe block)
                try {
                    if (get_option("is_new_manual_order_notice_email", 0) && ! $data_orders['mode']) {
                        $user_email    = $this->model->get("email", $this->tb_users, "id = '" . session('uid') . "'")->email;
