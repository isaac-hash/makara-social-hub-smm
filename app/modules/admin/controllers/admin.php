...
class admin extends My_AdminController
{
    ...
    public function login()
    {
        ...
        if (is_ajax_call()) {
            ...
            if ($item && $this->main_model->app_password_verify(post("password"), $item['password'])) {
                $error = false;
            } else {
                $error = true;
            }
            if (!$error) {
                ...
                insert_staffs_activity(['uid'=> $item['id'], 'event' => 'Authorization', 'detail' => 'Login'] );
    ...
    {
        insert_staffs_activity(['uid'=> session('sid'), 'event' => 'Authorization', 'detail' => 'Log out']);
        unset_session("sid");
