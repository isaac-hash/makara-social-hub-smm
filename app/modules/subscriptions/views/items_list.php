    ...
    <div class="orders-grid">
        <?php
            foreach ($items as $key => $item) :
                ...
                // In subscriptions, status 'error' is treated as 'pending'
                $item_status = (in_array($item['status'], ['error'])) ? 'pending' : $item['status'];
                
                // Extract order-like details from subscription item
                $order_quantity = $item['quantity'] ?? 0;
                $order_posts = $item['posts'] ?? 0; // Subscriptions have posts
                $order_delay = $item['delay'] ?? 0; // Subscriptions have delay
                $sub_username = $item['username'] ?? '';
        ?>
            <div class="order-card order-card_<?php echo esc($item['ids']); ?>">
                ...
                <div class="order-card-content">
                    ...
                    <!-- Expiry Date -->
                    <?php if (isset($item['expiry']) && !empty($item['expiry'])) : ?>
                    <div class="order-info-row">
                        <div class="order-icon order-icon-red">⌛</div>
                        <div class="order-info-content">
                ...
                <div class="order-card-footer">
                    ...
                    <div class="order-footer-item">
                        <span class="order-footer-number"><?php echo number_format($order_posts); ?></span>
                        <span class="order-footer-label">Posts</span>
                    </div>
                    <div class="order-footer-item">
                        <span class="order-footer-number"><?php echo number_format($order_delay); ?></span>
                        <span class="order-footer-label">Delay</span>
                    </div>
                </div>
            </div>
        <?php endforeach; ?>
