        ...
        if ($items_api) {
            ...
            foreach ($items_api as $key => $item_api) {
                ...
                if ($is_api_availible && !empty($current_provider_services) && empty($current_provider_services['error'])) {
                    ...
                    $log_message .= ' - Last Sync: ' . NOW;
                    // $data_item_api['description'] = 'Last Sync: ' . NOW;
                } else {
                    $log_message .= ' - Unable to connect to the API endpoint - ' . NOW;
                    // $data_item_api['status'] = 0;
                    // $data_item_api['description'] = 'Unable to connect to the API endpoint';
                }
                echo $log_message;
                ...
                //file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/provider_sync_service_log_a3erdss.txt', json_encode($data_item_api) . PHP_EOL, FILE_APPEND);
