...
class provider_model extends MY_Model
{
    ...
    public function list_items($params = null, $option = null)
    {
        ...
        if ($option['task'] == 'list-items') {
            $this->db->select('id, ids, name, url, key, status, description, no_current_services, created, balance, currency_code');
            ...
            insert_staffs_activity(['event' => 'Provider', 'detail' => 'View List']);
        }
    ...
    public function delete_item($params = null, $option = null)
    {
        ...
        if ($option['task'] == 'delete-item') {
            ...
            if ($item) {
                ...
                insert_staffs_activity(['event' => 'Delete Provider', 'detail' => $item->name]);
            } else {
                $result = [
                    'status'  => 'error',
    ...
    public function save_item($params = null, $option = null)
    {
        switch ($option['task']) {
            case 'add-item':
                ...
                insert_staffs_activity(['event' => 'Add Provider', 'detail' => $data['name']]);
            ...
            case 'edit-item':
                ...
                insert_staffs_activity(['event' => 'Edit Provider', 'detail' => $data['name']]);
            ...
            case 'balance-item':
                ...
                insert_staffs_activity(['event' => "Update Provider's balance", 'detail' => '#'. $params['id'] ]);
                return ["status" => "success", "message" => 'Update successfully'];
                break;
            ...
            case 'change-status':
                ...
                insert_staffs_activity(['event' => "Change Status Provider", 'detail' => '#'. $params['id']]);
                // Related Service
                if (! $params['status']) {
    ...
    public function crud_services($params = null, $options = null)
    {
        ...
        if ($options['task'] == 'bulk-import-service') {
            ...
            if (empty($api_services)) {
                return ["status" => "error", "message" => 'There seems to be an issue connecting to API provider. Please check API key and Token again!'];
            }
        ...
        if ($options['task'] == 'sync-services') {
            ...
            if (empty($params['items_provider_service'])) {
                return ["status" => "error", "message" => 'There seems to be an issue connecting to API provider. Please check API key and Token again!'];
            }
    ...
    private function sync_exists_services($data_services = [], $params = [])
    {
        ...
        if ($data_services) {
            ...
            foreach ($items_exists_db as $key => $item_db) {
                ...
                $sync_options['auto_status'] = isset($sync_request_options['status_with_provider']) ? 1 : 0;
                $sync_options['sync_rate'] = 1;
                $sync_options['auto_rate_percent'] = $price_percentage_increase;

                $sync_options = [
                    'auto_status' => isset($sync_request_options['status_with_provider']) ? 1 : 0,
                    'sync_rate' => 1,
                    'auto_rate_percent' => $price_percentage_increase,
                    'auto_sync_desc' => 0,
                    'auto_sync_name' => 0,
                    'sync_min' => 0,
                    'sync_max' => 0,
                ...
                if (isset($data_services[$item_db['api_service_id']])) {
                    ...
                    if (is_table_exists(ORDERS_REFILL) && isset($sync_request_options['refill']) && $sync_request_options['refill']) {
                        ...
                        $item_db['refill_type'] = (isset($item_exists_provider['refill']) && $item_exists_provider['refill_type']) ? $item_exists_provider['refill_type'] : 0;
                    }
                    /*---------- Cancel ----------*/
                    if (is_table_exists(ORDERS_CANCEL) && isset($sync_request_options['cancel']) && $sync_request_options['cancel']) {
                        $item_db['cancel']      = (isset($item_exists_provider['cancel']) && $item_exists_provider['cancel']) ? $item_exists_provider['cancel'] : 0;
                        $item_db['cancel_type'] = (isset($item_exists_provider['cancel']) && $item_exists_provider['cancel']) ? $item_exists_provider['cancel'] : 0;
                    }
                    /*---------- Old Service----------*/
                ...
                $item_db['sync_options'] = json_encode($sync_options);
    ...
    private function get_crud_data_service($service = [], $params = [], $task = 'add')
    {
        ...
        $data_item["sync_options"] = $this->prepare_sync_options($price_percentage_increase);
        if (is_table_exists(ORDERS_REFILL)) {
            $data_item["refill"]      = (isset($service['refill'])) ? (int) $service['refill'] : 0;
            $data_item["refill_type"] = (isset($service['refill'])) ? (int) $service['refill'] : 0;
        }
        if (is_table_exists(ORDERS_CANCEL)) {
            $data_item["cancel"]      = (isset($service['cancel'])) ? (int) $service['cancel'] : 0;
            $data_item["cancel_type"] = (isset($service['cancel'])) ? (int) $service['cancel'] : 0;
        }
        return $data_item;
    }

    private function prepare_sync_options($price_percentage_increase = 0) {

        if (!is_numeric($price_percentage_increase) || $price_percentage_increase <= 0) {
            return null;  
        }

        if ($price_percentage_increase > 0) {
            return json_encode([
                'sync_rate' => 1,
                'sync_min' => 1,
                'sync_max' => 1,
