<?php 
/**
 * Validates a phone number for both local and international formats.
 * 
 * This function checks if the provided phone number is valid based on the following rules:
 * 1. If the phone number starts with a '+', it ensures that it does not start with '0' after the '+' sign.
 * 2. If the phone number does not start with a '+', it must start with '0' (for local phone numbers).
 * 3. The phone number must have a length between 8 and 14 digits.
 * 4. The phone number is validated using regular expressions based on these rules.
 * 
...
if (!function_exists('validate_phone_number')) {
    function validate_phone_number($phone_number) {
        // Remove any unnecessary whitespace from the phone number
        $phone_number = trim($phone_number);

        $pattern = "/^\+?[0-9]{1,4}[0-9]{8,14}$/";
...
if (!function_exists('show_price_format')) {
    ...
    {
        $input_price = currency_format((double)$input_price);
        $curency_symbol = null;
...
if (!function_exists("freekassa_payment_setting")) {
    function freekassa_payment_setting()
    {
        $items = [
            [
                'id'   => 1, 'name' => 'FK WALLET RUB', 'active' => 1,
            ],
            [
                'id'   => 2, 'name' => 'FK WALLET USD', 'active' => 1,
            ],
            [
                'id'   => 3, 'name' => 'FK WALLET EUR', 'active' => 1,
            ],
            [
                'id'   => 4, 'name' => 'VISA RUB', 'active' => 1,
            ],
            [
                'id'   => 6, 'name' => 'Yoomoney', 'active' => 1,
            ],
            [
                'id'   => 7, 'name' => 'VISA UAH', 'active' => 1,
            ],
            [
                'id'   => 8, 'name' => 'MasterCard RUB', 'active' => 1,
            ],
            [
                'id'   => 9, 'name' => 'MasterCard UAH', 'active' => 1,
            ],
            [
                'id'   => 10, 'name' => 'Qiwi', 'active' => 1,
            ],
            [
                'id'   => 11, 'name' => 'VISA EUR', 'active' => 1,
            ],
            [
                'id'   => 12, 'name' => 'МИР', 'active' => 1,
            ],
            [
                'id'   => 42, 'name' => 'СПБ', 'active' => 1,
            ],
        ];
        return $items;
    }
