...
class auth extends My_UserController
{
    ...
    public function __construct()
    {
        ...
        // Google
        $this->social_login_google_enable = get_option("social_login_google_enable", '');
        $google_client_id = get_option('social_login_google_app_id');
        $google_client_secret = get_option('social_login_google_secret_key');
        if ($this->social_login_google_enable && $google_client_id != "" && $google_client_secret != "") {
    ...
    private function generate_social_login_html($type = 'login')
    {
        $button_title = lang('Sign_with_Google');

        if (isset($this->google_auth)) {
            $google_login_url = $this->google_auth->createLoginUrl();
        } else {
            $google_login_url = '';
        }
        $data = [
            'google_auth' => [
                'status' => $this->social_login_google_enable,
                'button_title' => $button_title,
                'url' => $google_login_url 
            ],
        ];
        return  $this->load->view('social_login', $data, TRUE);
    }

    public function callback($type) {
        if ($type == 'google') {
            $user_info = $this->google_auth->handleCallback();
            if ($user_info) {
                $email = $user_info->email;
                // check email
                $item_user = $this->model->get('id, status, ids, email, password, first_name, last_name, timezone', $this->tb_users, ['email' => $email, 'login_type' => 'google_login']);
                if (!empty($item_user)) {
    ...
    public function ajax_sign_up()
    {
        ...
        if (get_option("enable_signup_whatsapp_field", '')) {
            ...
            if (!$validate_whatsapp) {
                ms(array(
                    'status' => 'error',
                    'message' => lang("Whatsapp_is_not_a_valid_number"),
                ));
            }
    ...
    public function ajax_sign_in()
    {
        ...
        $user = $this->model->get("id, status, ids, email, password, first_name, last_name, timezone", $this->tb_users, ['email' => $email]);
