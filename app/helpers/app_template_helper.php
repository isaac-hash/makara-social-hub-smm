...
/**
 * Returns a list of searchable fields for filtering services.
 *
 * Each field includes:
 * - 'key': the logical identifier used in form selection and filtering logic.
 * - 'dataKey': the corresponding HTML data attribute (e.g., data-[key]) used in JavaScript.
 * - 'label': the display name shown in UI elements like dropdowns.
 *
 * @param array $only (Optional) An array of field keys to include.
 *                     If provided, only fields matching the keys will be returned.
 *                     If empty, all available fields will be returned.
 *
 * @return array An array of configured search field definitions.
 */
if (! function_exists('get_search_fields_config')) {
    function get_search_fields_config($only = [])
    {
        $all = [
            ['key' => 'id', 'dataKey' => 'id', 'label' => 'ID'],
            ['key' => 'name', 'dataKey' => 'name', 'label' => 'Name'],
            ['key' => 'api_service_id', 'dataKey' => 'api-service-id', 'label' => 'API Service ID'],
            ['key' => 'provider_name', 'dataKey' => 'provider-name', 'label' => 'Provider Name'],
        ];
        if (! empty($only)) {
            return array_filter($all, function ($f) use ($only) {
                return in_array($f['key'], $only);
            });
        }
        return $all;
    }
...
if (! function_exists('render_tooltip_popover_html')) {
    function render_tooltip_popover_html($content = '', $type = 'popover', $placement = 'right', $class_icon = 'fa fa-question-circle', $trigger = 'hover', $title = 'Details')
    {
        // Nếu là Popover
        if ($type == 'popover') {
            $xhtml = '<i class="%s" data-toggle="popover" data-html="true" data-trigger="%s" data-placement="%s" data-content="%s" data-title="%s"></i>';
            return sprintf($xhtml, $class_icon, $trigger, $placement, $content, $title);
        }
        // Nếu là Tooltip
        else {
            $xhtml = '<i class="%s" data-toggle="tooltip" data-placement="%s" title="%s"></i>';
            return sprintf($xhtml, $class_icon, $placement, $content);
        }
    }
}

/**
 * Renders HTML for a given text with an optional tooltip based on the active condition.
 *
 * This function allows displaying a tooltip before or after the main text based on the condition.
 * If the condition ($is_active) is true, it will show the tooltip; otherwise, it will just return the main text.
 *
 * @param string $main_text The main text to be displayed.
 * @param bool $is_active Condition to determine if the tooltip should be shown (default is false).
 * @param string $tool_tip_text The text to be displayed in the tooltip (default is 'Sync with provider').
 * @param string $tooltip_position The position of the tooltip relative to the text: 'before' or 'after' (default is 'before').
 * @param string $icon_class CSS classes for the tooltip icon (default is 'small fe fe-link text-info').
 * @return string The HTML for the main text with or without the tooltip.
 */
if (! function_exists('render_service_attr_html')) {
    function render_service_attr_html($main_text, $is_active = false, $tool_tip_text = 'Sync with provider', $tooltip_position = 'before', $icon_class = 'fs-6 fa fa-circle-o text-info')
    {
        if (! $is_active) {
            return $main_text;
        }
        $xhtml = render_tooltip_popover_html($tool_tip_text, 'tooltip', 'top', $icon_class);

        if ($tooltip_position === 'before') {
            return $xhtml . ' ' . $main_text;
        } else {
            return $main_text . ' ' . $xhtml;
        }
    }
}
 ...
 * @author Seji2906
 */
if (! function_exists('render_tr_no_item')) {
    function render_tr_no_item($text = '')
    {
        if (!$text) {
            $text = lang('look_like_there_are_no_results_in_here');
        }
        $xhtml = sprintf('<tr><td colspan="100" class="text-center">%s</td></tr>', $text);
        return $xhtml;
    }
}
...
if (! function_exists('show_pagination')) {
    function show_pagination($pagination, $position = 'float-right')
    {
        $xhtml = null;
        if (! empty($pagination)) {
            $xhtml .= sprintf('<div class="col-md-12"><div class="%s">%s</div></div>', $position, $pagination);
        }
        return $xhtml;
    }
}
...
if (! function_exists('show_item_button_action')) {
    function show_item_button_action($controller_name, $ids, $format = 'dropdown', $item_data = [], $params = [])
    {
        $xhtml           = null;
        ...
        if (in_array($controller_name, ['order', 'dripfeed', 'subscriptions'])) {
            $curent_btn_area = ['edit'];
            if ($controller_name == 'order') {
                if (in_array($item_data['status'], ['error', 'fail'])) {
                    $curent_btn_area[] = 'resend';
                } elseif ($item_data['status'] == ORDER_STATUS_COMPLETED && $item_data['api_order_id'] != -1) {
                    $curent_btn_area[] = 'update_from_provider';
                }
            }
        }

        $number_action_child = 0;
        switch ($format) {
            case 'btn-group':
                $xhtml .= '<div class="btn-group">';
                foreach ($curent_btn_area as $item) {

                    if (! staff_has_permission($controller_name, $item)) {
                        continue;
                    }

                    $current_btn = $tmpl_buttons[$item];
                    $link        = admin_url($controller_name . $current_btn['route-name'] . $ids);
                    if (isset($params['http_build_query'])) {
                        $link .= '?' . http_build_query($params['http_build_query']);
                    }
                    $confirm_message = "";
                    if ($item == 'delete') {
                    ...
                    $xhtml .= sprintf(
                        '<a href="%s" class="btn btn-icon btn-outline-primary %s" data-confirm_ms="%s" data-toggle="tooltip" data-placement="bottom" title="%s">
                            <i class="%s"></i>
...
if (! function_exists('show_bulk_btn_action')) {
    function show_bulk_btn_action($controller_name, $params = '')
    {
        $xhtml           = null;
        ...
        foreach ($curent_btn_area as $item) {
            ...
            if ($controller_name == 'order') {
                if (in_array($item, ['pending', 'inprogress', 'completed'])) {
                    $permission_type = 'change_status';
                }
            }
           
            if (in_array($controller_name, ['users', 'subscribers', 'services', 'category', 'blacklist_ip', 'blacklist_link', 'blacklist_email'])) {
                if (in_array($item, ['active', 'deactive'])) {
                    $permission_type = 'change_status';
                }
                if (in_array($item, ['csv', 'excel'])) {
                    $permission_type = 'export';
                }
            }

            if (! staff_has_permission($controller_name, $permission_type) && in_array($permission_type, ['change_status', 'export', 'unread', 'delete', 'closed', 'cancel', 'copy_api_order_id', 'copy_api_refill_id', 'resend', 'empty'])) {
                continue;
            }
    ...
    function show_item_status($controller_name, $id, $status, $type = null, $task = null)
    {
        ...
        switch ($type) {
            ...
            default:
                ...
                if (in_array($controller_name, ['order', 'dripfeed', 'subscriptions', 'refill', 'affiliates', 'cancel'])) {
...
if (! function_exists('show_filter_status_button')) {
    function show_filter_status_button($controller_name, $items_status_button = '', $params = '')
    {
        $xhtml         = null;
        $config_status = app_config('config')['status'];
        if (in_array($controller_name, ['order', 'dripfeed', 'subscriptions', 'refill', 'cancel'])) {
            ...
            $xhtml .= '<div class="btn-group btn-filter-status m-b-10">';
...
if (! function_exists('show_item_ticket_subject')) {
    function show_item_ticket_subject($controller_name, $item_data, $params = [])
    {
        ...
        if (staff_has_permission($controller_name, 'view')) {
...
if (! function_exists('show_item_ticket_message_detail')) {
    function show_item_ticket_message_detail($controller_name, $item = [], $task = '')
    {
        $xhtml        = null;
        $xhtml_footer = null;
        if ((isset($item['support']) && $item['support']) || (isset($item['created_by']) && $item['created_by'])) {
            $class_item = 'replies tr_' . $item['ids'];
            ...
            if ($task == 'user') {
                ...
                $xhtml_delete_btn = null;
                if (staff_has_permission($controller_name, 'delete_message')) {
                    $delete_item_link = admin_url($controller_name . '/delete_item_ticket_message/' . $item['ids']);
                    $xhtml_delete_btn = sprintf(
                ...
                $xhtml_footer     = sprintf(
                    '<div class="msg-footer p-t-5">
                        %s
                        %s
                    </div>', $xhtml_edit_btn, $xhtml_delete_btn
                );
            }
        } else {
        ...
        $content = nl2br(esc($item['message']));
        $created = show_item_datetime($item['created'], 'long');

        $author = (isset($item['created_by']) && $item['created_by']) || isset($item['support']) && $item['support'] ? 'Support Team' : $item['first_name'] . ' ' . $item['last_name'];
...
if (! function_exists('show_view_ticket_button_group')) {
    function show_view_ticket_button_group($controller_name, $item = [])
    {
        ...
        if (staff_has_permission($controller_name, 'closed')) {
            $xhtml_close_btn = sprintf(
                '<a href="%s" class="btn btn-outline-primary">Close ticket</a>', $closed_link
            );
        } else {
            $xhtml_close_btn = '<btn class="btn btn-primary">Change Status</btn>';
        }
        ...
        if ($dropdowns) {
            ...
            foreach ($dropdowns as $key => $dropdown) {
                if (! staff_has_permission($controller_name, 'unread') && $key == 'unread') {
                    continue;
                }
        ...
        $xhtml = sprintf(
            '<div class="btn-group" role="group" aria-label="Actions Group">
                %s
                %s
            </div>', $xhtml_close_btn, $xhtml_dropdown);
        return $xhtml;
    }
}
...
if (! function_exists('show_items_sort_by_category')) {
    function show_items_sort_by_category($controller_name, $items = [], $params = '')
    {
        ...
        $xhtml = sprintf(
            '<select name="status" class="form-select search-by-category m-t-4 m-r-1">
...
if (! function_exists('show_item_order_details')) {
    function show_item_order_details($controller_name, $item = [], $params = [], $task = 'admin')
    {
        ...
        switch ($controller_name) {
            ...
            case 'dripfeed':
                ...
                $order_attrs = [
                    ...
                    'total_charge'   => (double) $item['charge'],
                    'runs'           => $runs,
            ...
            case 'refill':
                $order_attrs = [
                    ...
                    'start_counter' => ($item['start_counter']) ? $item['start_counter'] : '',
                ];
                break;
            ...
            default:
                ...
                if ($item['formal_charge'] != 0 && staff_has_permission($controller_name, 'see_cost')) {
        ...
        // Permission
        if (isset($order_attrs['charge']) && ! staff_has_permission($controller_name, 'see_charge')) {
            unset($order_attrs['charge']);
        }
        if (isset($order_attrs['total_charge']) && ! staff_has_permission($controller_name, 'see_charge')) {
            unset($order_attrs['total_charge']);
        }
        if (isset($order_attrs['provider']) && ! staff_has_permission($controller_name, 'copy_api_order_id')) {
            unset($order_attrs['provider']);
        }

        $xhtml_order_attr = null;
        $xhtml_order_attr = '<ul style="margin: 0px;">';
        foreach ($order_attrs as $key => $attr) {
            $attr_content = ucfirst(str_replace('_', ' ', $key)) . ': ' . $attr;
...
if (! function_exists('show_item_order_id')) {
    function show_item_order_id($controller_name, $item = [], $params = [])
    {
        ...
        $api_order_id = '';
        if ($item['api_order_id'] > 0 && staff_has_permission($controller_name, 'copy_api_order_id')) {
...
if (! function_exists('show_item_service_attr')) {
    function show_item_service_attr($item = [])
    {
        ...
        if (is_table_exists(ORDERS_CANCEL) && isset($item['cancel']) && $item['cancel']) {
            if ($item['cancel_type']) {
                $cancel_title = 'Cancel allowed';
                $cancel_class = '';
            } else {
                $cancel_title = 'Manual cancel allowed';
                $cancel_class = 'text-primary';
            }
            $xhtml .= ' <span class="fe fe-slash ' . $cancel_class . '" data-toggle="tooltip" data-placement="top" title="" data-original-title="' . $cancel_title . '"></span>';
        }
        if (is_table_exists(ORDERS_REFILL) && $item['refill']) {
...
if (! function_exists('get_table_class')) {
    ...
    {
        $template = app_config('template')['table']['class'] ?? 'table table-hover table-borderless table-vcenter card-table';
        return $template;
    }
}
